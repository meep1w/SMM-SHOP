...
class User(Base):
    ...
    seq = Column(Integer, nullable=False, index=True)  # «реф-код»
    nick = Column(Text)
...
class Topup(Base):
    ...
    applied = Column(Boolean, nullable=False, default=False)  # списано ли на баланс пользователя
    pay_url = Column(Text)
    created_at = Column(Integer, default=now_ts)
    updated_at = Column(Integer, default=now_ts)
...
class Referral(Base):
    ...
    id = Column(BigInteger, primary_key=True, autoincrement=True)
    referrer_id = Column(BigInteger, ForeignKey("users.id", ondelete="CASCADE"), index=True)
    referred_id = Column(BigInteger, ForeignKey("users.id", ondelete="CASCADE"), index=True)
    created_at = Column(Integer, default=now_ts)
    __table_args__ = (
        UniqueConstraint("referred_id", name="uq_referrals_referred_id"),  # один пригласитель на пользователя
    )
...
class ReferralPayout(Base):
    ...
    referrer_id = Column(BigInteger, ForeignKey("users.id", ondelete="CASCADE"), index=True)
    referred_id = Column(BigInteger, ForeignKey("users.id", ondelete="CASCADE"), index=True)
    topup_id = Column(BigInteger, ForeignKey("topups.id", ondelete="CASCADE"), unique=True)  # по одному начислению на пополнение
    amount_usd = Column(Numeric(18, 6), nullable=False)   # сумма бонуса в USD
    rate_percent = Column(Integer, nullable=False)        # 10 или 20
    created_at = Column(Integer, default=now_ts)

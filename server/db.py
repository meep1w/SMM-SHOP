...
class Topup(Base):
    ...
    applied = Column(Boolean, nullable=False, default=False)  # зачислено пользователю
    pay_url = Column(Text)
    created_at = Column(Integer, default=now_ts)
    updated_at = Column(Integer, default=now_ts)
...
class RefLink(Base):
    """
    Кто кого пригласил.
    Один пользователь может иметь только одного пригласителя.
    """
    __tablename__ = "ref_links"
    user_id = Column(BigInteger, ForeignKey("users.id", ondelete="CASCADE"), primary_key=True)  # реферал
    referrer_id = Column(BigInteger, ForeignKey("users.id", ondelete="CASCADE"), nullable=False, index=True)
    created_at = Column(Integer, default=now_ts)

class RefReward(Base):
    """
    Начисление бонусов пригласителю за конкретный топап реферала.
    """
    __tablename__ = "ref_rewards"
    id = Column(BigInteger, primary_key=True, autoincrement=True)
    referrer_id = Column(BigInteger, ForeignKey("users.id", ondelete="CASCADE"), nullable=False, index=True)
    topup_id = Column(BigInteger, ForeignKey("topups.id", ondelete="CASCADE"), nullable=False, unique=True)
    amount_usd = Column(Numeric(18, 6), nullable=False)
    currency = Column(Text, nullable=False, default="USD")
    applied = Column(Boolean, nullable=False, default=False)  # зачислено на баланс пригласителя
    created_at = Column(Integer, default=now_ts)

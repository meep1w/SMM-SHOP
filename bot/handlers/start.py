...
async def send_main_menu(m: Message | CallbackQuery, nick: str | None = None):
    text = (
        f"Привет{',' if nick else ''} <b>{html.escape(nick) if nick else m.from_user.full_name}</b>! это магазин SlovekinzShop\n\n"
        f"Продвигай свои соц.сети, каналы и воронки по лучшим ценам с лучшим сервисом в любое время!\n"
        f"Так же вы можете пообщаться в моем <a href=\"{html.escape(PUBLIC_CHAT_URL or GROUP_URL or '#')}\">открытом чате</a>,"
        f"либо зайти в мой <a href=\"{html.escape(GROUP_URL or '#')}\">телеграм канал</a> на тематику Арбитража Трафика"
    )
